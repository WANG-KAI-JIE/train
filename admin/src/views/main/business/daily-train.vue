  ...
  >
    ...
    >
      ...
      <a-form-item label="车次编号">
        <train-select-view v-model:value="dailyTrain.code" @click="onChange" />
      </a-form-item>
      ...
      <a-form-item label="停站时长">
        <a-time-picker
          v-model:value="dailyTrain.stopTime"
          valueFormat="HH:mm:ss"
          placeholder="请选择时间"
          disabled
        />
...
watch(
  () => dailyTrain.value.start,
  () => {
    if (Tool.isNotEmpty(dailyTrain.value.start)) {
      dailyTrain.value.startPinyin = pinyin(dailyTrain.value.start, {
        toneType: "none",
      }).replaceAll(" ", "");
    } else {
...
watch(
  () => dailyTrain.value.end,
  () => {
    if (Tool.isNotEmpty(dailyTrain.value.end)) {
      dailyTrain.value.endPinyin = pinyin(dailyTrain.value.end, {
        toneType: "none",
      }).replaceAll(" ", "");
    } else {
...
// 自动计算停车时长
watch(
  () => dailyTrain.value.startTime,
  () => {
    let diff = dayjs(dailyTrain.value.endTime, "HH:mm:ss").diff(
      dayjs(dailyTrain.value.startTime, "HH:mm:ss"),
      "seconds"
    );
    dailyTrain.value.stopTime = dayjs("00:00:00", "HH:mm:ss")
      .second(diff)
      .format("HH:mm:ss");
  },
  { immediate: true }
